...
generation_dt <- fread(data_file) #data.table read fxn. 

#datatables only output first/last 5lines when put to console
...
# data.table
generation_dt[,.(datetime, small_hydro)] #same as [,list(datetime,small_hydro)]
...
#results in a new datatable and i can assign it to new data.table
...
# data.table
generation_dt[,all_hydro := small_hydro + large_hydro] #create a column that is := and add it to current datatable

#does this in place, so it adds it to my current table. adds the variable WITHOUT having to assign anything. this is changing the object we are currently working wiht, implicit assignment. 
...
# two steps
generation_dt[solar > 0 & wind >2500, solar_on := TRUE] #will assign a new column where TRUE is true if boolean condition is true. 
...
#generation_dt[,.(datetime= as_datetime(datetime), day= day(datetime), hour=hour(datetime))]
#generation_dt[,:=(datetime= as_datetime(datetime), day= day(datetime), hour=hour(datetime))]

generation_dt[,datetime:=as_datetime(datetime)] #inplace changing datetime to a datetime obj. 
#generation_dt[,day:= day(datetime), hour=hour(datetime)]
#generation_dt[,hour:=hour(datetime)]


generation_dt[,.(solar_wind = solar + wind), by = .(day(datetime), hour(datetime))]
#by is sorta like groupby. 
```
