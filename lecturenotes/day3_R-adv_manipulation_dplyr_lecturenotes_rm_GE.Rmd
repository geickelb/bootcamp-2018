...
dim(melted_merged_generation) #1680 by 3, went from 168->1680, so 10 variables collapsed with values in each. 
head(melted_merged_generation)
...
```{r}
#select is just for variables. 
tmp<- select( merged_energy, contains('hydro'), starts_with("bio")) #chaining helpers uses or, need regex to use and / nor

#can use matches and regular expressions to make very powerful data selection tools. 

names(tmp)
```

## `filter` - used for observations based on conditions (ie looks at values in cells)

```{r filter}
tmp <- filter(merged_energy, imports > 7000) #works like subset. return data subset where imports > 7000. 
nrow(tmp)
head(tmp)
```
...
tmp <- filter(merged_energy, imports > 7000 | natural_gas < 7000) #or

tmp <- filter(merged_energy, imports > 7000 & natural_gas < 7000) #same as first. 
...
```{r mutate}
#base r:   long_merged_energy$log_usage <- log(long_merged_energy$usage) 
...
```{r multi mutate}
#chaining together lots of new derived rows
...
##be sure to name the summarized variable. 
...
merged_energy %>% 
  ...
  summarize(mean_hydro= mean(total_hydro, na.rm=T)) #remember row above is the whole df + new col so i need to specify
...
# long_merged_energy %>% 
#   filter(source, contains("hydro"), contains("bio")) %>%
#   group_by(source) %>%
#   summarize(mean_usage = mean(usage, na.rm=T))
```
